...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("Piplup Step",
...
void process_input()
{
    ...
    if (g_current_scene->get_state().player->get_position().x >= 38.5f  && g_current_scene->get_state().player->get_position().y >= -2.0f && g_current_scene != g_level_c)
    {
        printf("lol");
        Mix_PlayChannel(-1,g_current_scene->get_state().hurt_sfx, 0);
        Mix_VolumeChunk(g_current_scene->get_state().hurt_sfx, 10);
    ...
    while (SDL_PollEvent(&event))
    {
            ...
            g_current_scene->get_state().player->get_gameover())
        {
            ...
            if(g_current_scene->get_state().player->get_gameover())
            {
                Mix_PlayChannel(1,g_current_scene->get_state().hurt_sfx, 0);
            }
    ...
    if (key_state[SDL_SCANCODE_LEFT] && !(g_current_scene->get_state().player->get_win() ||
                                          g_current_scene->get_state().player->get_gameover()))
        g_current_scene->get_state().player->move_left();
    else if (key_state[SDL_SCANCODE_RIGHT] && !(g_current_scene->get_state().player->get_win() ||
                                             g_current_scene->get_state().player->get_gameover()))
