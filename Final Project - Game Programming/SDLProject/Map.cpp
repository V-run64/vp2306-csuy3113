...
Map::Map(int width, int height, unsigned int *level_data, GLuint texture_id, float tile_size, int tile_count_x, int tile_count_y) :
    m_width(width), m_height(height), m_texture_id(texture_id), m_tile_size(tile_size),
    m_tile_count_x(tile_count_x), m_tile_count_y(tile_count_y)
{
    m_original_level_data = std::vector<unsigned int>(level_data, level_data + width * height);
    m_level_data = new unsigned int[width * height];
    std::copy(m_original_level_data.begin(), m_original_level_data.end(), m_level_data);
...
void Map::build()
{
    ...
    for(int y_coord = 0; y_coord < m_height; y_coord++)
    {
        for(int x_coord = 0; x_coord < m_width; x_coord++)
        {
            ...
            if (tile == 0 && m_level_data[y_coord * m_width + x_coord+1] !=1) continue;
...
void Map::rebuild_mesh()
{
    // Clear existing mesh data
    m_vertices.clear();
    m_texture_coordinates.clear();
    
    // Rebuild from current level data
    for(int y_coord = 0; y_coord < m_height; y_coord++)
    {
        for(int x_coord = 0; x_coord < m_width; x_coord++)
        {
...
void Map::destroy_tile_at(glm::vec3 position)
{
    ...
    // Check bounds
    if (tile_x < 0 || tile_x >= m_width) return;
    if (tile_y < 0 || tile_y >= m_height) return;
    
    // Destroy the tile (set to 0)
...
bool Map::is_solid(glm::vec3 position, float *penetration_x, float *penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    if (tile == 0 && m_level_data[tile_y * m_width + tile_x+1] !=1) return false;
    if (tile == 4) return false;
    if (tile == 18 && m_level_data[tile_y-1 * m_width + tile_x-1]!=100) return false;
    if (tile == 37 && m_level_data[tile_y * m_width + tile_x+1]!=0 && m_level_data[tile_y * m_width + tile_x+1]!=361) return false;
    if (tile == 29) return false;
    if (tile == 27 && m_level_data[tile_y * m_width + tile_x-1]!=0 ) return false;
    if (tile == 20) return false;
    if (tile == 11) return false;
    if (tile == 109) return false;
