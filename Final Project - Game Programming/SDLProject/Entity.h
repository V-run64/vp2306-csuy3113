...
enum AnimationDirection { LEFT, RIGHT, UP, DOWN, NE, SE, NW, SW };
...
public:
    ...
    // ————— METHODS ————— //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, int walking[8][8], float animation_time,
    ...
    void face_ne() { m_animation_indices = m_walking[NE];}
    ...
    void const scare() { m_is_scaring = true;}
    void const hold() { m_is_holding = true;}
    void const scare_off() { m_is_scaring = false;
        m_is_holding = false;
    ...
    void set_walking2(int walking[8][8])
    {
        for (int i = 0; i < 4; ++i)
        {
            for (int j = 0; j < 8; ++j)
